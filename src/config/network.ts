...
export const isTestnet = CURRENT_NETWORK === NETWORK_CONFIG.TESTNET;
export const isDevnet = CURRENT_NETWORK === NETWORK_CONFIG.DEVNET;

// ============================================================================
// TOKEN CONFIGURATION
// ============================================================================

const TOKEN_NAME: string = isMainnet ? 'tri' : 'trie';
const SHOW_REQUEST_TOKENS_BUTTON: boolean = isTestnet;
...
const CONTRACT_ADDRESSES: ContractAddresses = {
  ...
  TOPUP_TOKEN: isMainnet ? "" : "QmRQb5213qyG7xiZS2sF9WqPvpBjb34jpGFLSoTAeeqEcH"
} as const;
...
const API_ENDPOINTS: ApiEndpoints = {
  ...
  comp: isMainnet ? "" : 
        isTestnet ? "https://dev-api.xellwallet.com:8448" :
...
const networkUtils: NetworkUtility = {
  ...
  validateNetworkConfig: (): boolean => {
    // Simple validation - just check if we have a valid network
    const validNetworks = ['mainnet', 'testnet', 'devnet'];
    if (!validNetworks.includes(CURRENT_NETWORK)) {
  ...
  getNetworkInfo: (): NetworkInfo => {
    return {
      ...
      environment: isMainnet ? 'production' : isTestnet ? 'test' : 'development',
      features: {
        ...NETWORK_INFO.features,
        mainnetFeatures: isMainnet,
        faucet: isTestnet || isDevnet,
        competitions: isTestnet || isDevnet,
        devnetFeatures: isDevnet
      }
    };
  },
...
export { TOKEN_NAME, SHOW_REQUEST_TOKENS_BUTTON };
